...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
   ...
   // Zeig den Splash-Screen:
	...
	{
		MSG msg;
		if (PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) // if => Pro Frame wird eine Message verarbeitet, while =>  => Pro Frame werden alle anstehenden Messages verarbeitet. Achtung, zweiter Parameter muss NULL sein!
		{
		...
		{
			//FrameRate
			timer.Tick(fTime, fTimeDelta, g_game.GetTimeDeltaMin());
			...
			{
				char ac[300];
				sprintf_s(ac, 300, "Vektoria V%2.2f FR-Average: %.3f; FR-Long Average: %.3f; FR Min: %.3f; FR Max: %.3f", g_game.GetVersion(),timer.m_fFrameRateAverage, timer.m_fFrameRateAverageLong, timer.m_fFrameRateMin, timer.m_fFrameRateMax);
				SetWindowTextA(hwnd, ac);
				fTimeLastShow = fTime;
			}
