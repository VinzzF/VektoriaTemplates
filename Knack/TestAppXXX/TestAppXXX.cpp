// ------------------------------------------------------------------------
// TestAppXXX.cpp : Definiert den Einstiegspunkt für die Konsolenanwendung
   ...
   // Zeig den Splash-Screen:
	...
	do
	{
		...
		else
		{
			...
			{
				char ac[300];
				sprintf_s(ac, 300, "TestAppXXX        FR-Average: %.3f; FR-Long Average: %.3f; FR Min: %.3f; FR Max: %.3f", timer.m_fFrameRateAverage, timer.m_fFrameRateAverageLong, timer.m_fFrameRateMin, timer.m_fFrameRateMax);
				SetWindowTextA(hwnd, ac);
				fTimeLastShow = fTime;
			}
